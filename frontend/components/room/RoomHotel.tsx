...
const RoomHotel = ({hotelId, onLoading}: Props) => {
    const services = [
        'Free toiletries',
        'Bidet',
        'Toilet',
        ...
        <>
            {total > 0 ? (
                <div className="text-sm lg:text-base text-primary flex flex-col gap-y-0.5">
                    ...
                    <h2 className="text-xl lg:text-3xl font-semibold">SOL {price.toFixed(3)}</h2>
    ...
    const booking = async () => {
        if (!checkIn || !checkOut || !bookingBody.roomId || !price) {
            ...
            onLoading(true);
            const result = await deposit(connection, anchorWallet, price);
            if (!result) {
                        ...
                        {rooms?.map((room) => (
                            <div key={room._id} className="grid mt-2.5 md:mt-0 md:grid-cols-12">
                                ...
                                <div
                                    ...
                                    <div>
                                        <p className="font-semibold text-base 2xl:text-xl break-all">SOL{room.price}</p>
